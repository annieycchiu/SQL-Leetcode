...
with base as (
    select tb1.activity, count(tb1.name) cnts,
           max(count(tb1.name)) over () max_cnts, 
           min(count(tb1.name)) over () min_cnts
    from Friends tb1
    left join Activities tb2
    on tb1.activity = tb2.name
    group by tb2.id
)
...
select activity
from base
where cnts != max_cnts and cnts != min_cnts